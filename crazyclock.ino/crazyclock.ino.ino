...
void loop() {
  ...
  void ticTac() {
    ...
    if (myMillis=millis() && tick>0){
      ...
      mS--;} //if our second has passed and tick is minus, decrement;
    
    if (mS=60){
      mS=mS-60;
      if(tick>0){mM++;//if second has passed and tick is plus, increment minute
    } }
    if (mS<0){
      mS=mS+60;
    ...
    if (mM=60){
      mM=mM-60;
      if(tick>0){mH++;//if minute has passed and tick is plus, increment hour
    } }
    if (mM<0){
      mM=mM+59;
